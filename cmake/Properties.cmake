...
function(_doxypress_property_update _property)
    ...
    if (_value STREQUAL "" OR _overwrite)
        _doxypress_property_apply_setters(${_property} _value)
    else ()
...
function(_doxypress_apply_handler _property _type _name _value _overwrite)
    if (_type STREQUAL "setter")
        if (_value STREQUAL "" or _overwrite)
            # call setter
            _doxypress_log(DEBUG "call setter ${_name}")
            _doxypress_action(${_property} setter "${_value}")
            _doxypress_call(_doxypress_${_name} "${_value}" _value)
        endif()
    elseif(_type STREQUAL "updater")
        # call updater
        _doxypress_log(DEBUG "call updater ${_name}")
        _doxypress_action(${_property} updater "${_value}")
        _doxypress_call(_doxypress_${_name} "${_value}" _value)
    elseif(_type STREQUAL "default")
        if (_value STREQUAL "")
            # if no default, _value is left empty
            if (NOT _name STREQUAL "")
                set(_value "${_name}")
                _doxypress_action(${_property} default "${_value}")
                _doxypress_log(DEBUG "[default] ${_property} = ${_name}")
            endif ()
        endif ()
    endif()
endfunction()