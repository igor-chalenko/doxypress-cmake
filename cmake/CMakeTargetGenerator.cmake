...
#
# * ``<prefix>.doxypress`` invokes `DoxyPress`;
# * ``<prefix>.doxypress.open_html`` opens the HTML files generated
# by the `DoxyPress` target; this target is created if HTML generation was
# enabled;
# * ``<prefix>.doxypress.open_latex`` opens the file ``refman.tex``, which is
# generated by the `DoxyPress` target; this target is created if LaTex
# generation was enabled;
# * ``<prefix>.doxypress.open_pdf`` opens the file ``refman.pdf``, which
# generated by the `DoxyPress` target; this target is created if PDF
# generation was enabled.
#
...
function(_doxypress_targets_create _project_file _updated_project_file)
    ...
    if (NOT TARGET "${_doxypress_target}")
        _doxypress_target_docs("${_doxypress_target}" "${_output_dir}")
        _doxypress_target_pdf("${_doxypress_target}" "${_output_dir}")
        if (DOXYPRESS_ADD_OPEN_TARGETS)
            _doxypress_targets_open_files("${_doxypress_target}" "${_output_dir}")
        endif ()
    endif ()
endfunction()
